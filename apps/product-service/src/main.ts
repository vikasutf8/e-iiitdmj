...
app.get('/', (req, res) => {
  res.send({ message: 'Hello API  with api gateway proxy pass product service' });
});
...
const server = app.listen(port, () => {
  console.log(`[ ready ] Product service listening at http://localhost:${port}`);
  // console.log("Swagger UI at http://localhost:6001/docs-json");
})


server.on('error', (err)=>{
  console.log("product service error",err);
});